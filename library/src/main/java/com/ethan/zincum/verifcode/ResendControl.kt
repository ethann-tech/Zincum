        ...
        counter = ResendCounter.create(handler, 60, 0, -1, 1000, object : ResendCounter.Listener {
            override fun update(count: Int) {
                ...
                resendHelper.resendUpdate(
                    textColor, if (enableBackground == 0) R.drawable.bg_btn_gray_full_14_shape else enableBackground
                )
            }

            override fun complete() {
                isCountDown = false // 倒计时完成删除号码更改显示
                if (TextUtils.isEmpty(editText.text)) {
                    tvSend.text = editText.context.getString(R.string.get_verification_code) //edtTextView文本为空不能点击
                    resendHelper.enable(
                        textColor, if (enableBackground == 0) R.drawable.bg_btn_gray_full_14_shape else enableBackground
                    )
                } else {
                    if (editText.text.toString() == tmpNum) { //对比上次获取验证码的号码，如果一样则显示重新获取，否则显示获取验证码
                    ...
                    resendHelper.getCode(
                        textColor, if (selectBackground == 0) R.drawable.bg_btn_orange_full_14_shape else selectBackground
                    )
                }
            }
        })
